...
public class TestAccountTransactionGenerator {
	...
	Location randomLocation() {
		boolean hasPostal = (rand.nextDouble() < 0.3d);
		boolean hasMap = (rand.nextDouble() < 0.2d);
		boolean hasDesc = (rand.nextDouble() < 0.7);

		if (!(hasPostal || hasMap || hasDesc))
			hasPostal = true;

		PostalLocation pLoc = hasPostal ? createPostalLocation(rand.nextLong() + " main st", "section", "estate",
				"village", "sub location", "location", "district", "division", "province", "08893") : null;
		MapLocation mLoc = hasMap ? createMapLocation(rand.nextDouble() * 180d, rand.nextDouble() * 180d) : null;
		DescriptiveLocation dLoc = hasDesc ? createDescriptiveLocation("third box on the left", "uncle bens") : null;

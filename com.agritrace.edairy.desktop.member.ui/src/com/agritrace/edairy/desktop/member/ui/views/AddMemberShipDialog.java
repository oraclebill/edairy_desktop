...
public class AddMemberShipDialog extends TitleAreaDialog implements ModifyListener {
	...
	private boolean validate() {
		boolean valid = true;
		// check name
		valid = memberShip.getMember().getGivenName() != null
				&& !memberShip.getMember().getGivenName().trim().isEmpty();
		if (!valid) {
			return false;
		}
		valid = memberShip.getMember().getFamilyName() != null
				&& !memberShip.getMember().getFamilyName().trim().isEmpty();
		if (!valid) {
			return false;
		}
		// check address
		final String address = memberShip.getMember().getLocation().getPostalLocation().getAddress();
		valid = address != null && !address.trim().isEmpty();
		if (!valid) {
			return false;
		}
		// check section
		final String section = memberShip.getMember().getLocation().getPostalLocation().getSection();
		valid = section != null && !section.trim().isEmpty();
		if (!valid) {
			return false;
		}
		// check village
		final String village = memberShip.getMember().getLocation().getPostalLocation().getVillage();
		valid = village != null && !village.trim().isEmpty();
		if (!valid) {
			return false;
		}
		// check postal code
		final String postal = memberShip.getMember().getLocation().getPostalLocation().getPostalCode();

...
public class MemberInfoGroup  {
	...
	public void initGUI(){
		...
		Composite upperPanel = UIControlsFactory.createComposite(composite);
...
//		txtLast.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false,2, 1));

		photoLabel = UIControlsFactory.createLabel(upperPanel, ""); //$NON-NLS-1$
		photoLabel.setImage(EDairyActivator.getImage(ImageRegistry.sample_memberphoto));
		...
		//		GridDataFactory.swtDefaults().align(SWT.FILL, SWT.FILL).grab(true,false).span(6, 1).applyTo(datePanel);
		//		datePanel.setLayout(new GridLayout(8,false));

		UIControlsFactory.createLabel(upperPanel, applicationDate);
		dateText = UIControlsFactory.createText(upperPanel, SWT.READ_ONLY|SWT.BORDER,ViewWidgetId.memberInfo_applicationDate);
		...
		Button calendarButton = new Button(upperPanel,SWT.PUSH);
		...
		Button calendarButton2 = new Button(upperPanel,SWT.PUSH);
		...
		Group addressGroup = UIControlsFactory.createGroup(upperPanel, "Address Information");
...
//		txtRoute.setLayoutData(new GridData(SWT.FILL, SWT.TOP, false, false,2, 1));
//
...
//			}
//		});
//		GridDataFactory.fillDefaults().grab(true, true).span(2, 2).applyTo(listStops);
	...
	private void createAddresscontrol(Composite parent){
		...
		Text txtDistrict = UIControlsFactory.createText(addressPanel,	SWT.BORDER, ViewWidgetId.DISTRICT_TXT);
		...
		Combo comboProvince = UIControlsFactory.createCombo(addressPanel, ViewWidgetId.PROVINCE_TXT);
		...
		Text txtPostal = UIControlsFactory.createText(addressPanel, SWT.BORDER,ViewWidgetId.POSTAL_CODE_TXT);
		txtPostal.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,false, 2, 1));
	}


	private void initDataBinding(){
		bindingContext.bindValue(SWTObservables.observeText(txtId, SWT.FocusOut), EMFObservables.observeValue(memberShip,DairyPackage.Literals.MEMBERSHIP__MEMBER_ID), null, null);
		SimpleFormattedDateBean bean = new SimpleFormattedDateBean();
...
//		WritableList input = new WritableList(memberShip.getDefaultRoute().getStops(),CollectionCentre.class);

...
import com.agritrace.edairy.desktop.ui.views.data.MilkCollectionRecord;
...
public class ModifyMilkRecordDialog extends TitleAreaDialog implements ModifyListener, SelectionListener {
	...
	final Label lineLabel = new Label(dialogArea, SWT.NULL);
	...
	final Label memberLabel = new Label(dialogArea, SWT.NULL);
	...
	final Label binLabel = new Label(dialogArea, SWT.NULL);
	...
	final Label quantityLabel = new Label(dialogArea, SWT.NULL);
	...
	if (e.getSource() == quantityText) {
	    final String text = quantityText.getText();
	    try {
		final Double value = new Double(text);

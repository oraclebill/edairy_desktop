...
public class FinanceNavigationNodeAssembler extends AbstractNavigationAssembler {
	...
	public INavigationNode<?>[] buildNode(NavigationNodeId nodeId, NavigationArgument navigationArgument) {
		...
		ISubApplicationNode subAppNode = new SubApplicationNode(new NavigationNodeId(SUBAPP_FINANCE), LABEL_FINANCE);
		WorkareaManager.getInstance().registerDefinition(subAppNode, TAB_FINANCE);
		
		IModuleGroupNode moduleGroupNode = new ModuleGroupNode(new NavigationNodeId(MODULE_GROUP_FINANCE));
		...
		final IModuleNode financeMembers = NodeFactory.createModule(MODULE_FINANCE, LABEL_FINANCE, moduleGroupNode); //$NON-NLS-1$ 

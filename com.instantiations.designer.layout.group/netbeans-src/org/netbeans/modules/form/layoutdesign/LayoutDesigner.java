...
//                            assert LayoutInterval.isAlignedAtBorder(parComp, interval.getParent(), first ? LEADING : TRAILING);
    ...
    private void paintAlignment(IFeedbacksDrawer g, LayoutInterval interval, int dimension, int alignment) {
        LayoutInterval parent = interval.getParent();
    ...
    private int prefSizeOfInterval(LayoutInterval interval) {
        ...
        if (interval.isComponent()) {
            ...
            if (comp.isLinkSized(dimension)) {
                Collection linked = layoutModel.getLinkSizeGroups(dimension).get(new Integer(comp.getLinkSizeId(dimension)));

...
import com.agritrace.edairy.desktop.operations.services.dairylocation.*;
...
public class DairyLocationController extends SubModuleController {
	...
	final private DairyLocationRepository locationRepository = new DairyLocationRepository();

	final private IObservableList locations = new WritableList();
	final private IObservableList routes = new WritableList();
...
//			});
	...
	private final class DairyLocationDelegate extends
			AbstractMasterDetailsDelegate {
		private DairyLocation workingCopy = (DairyLocation) createWorkingCopy();
		...
		private WritableValue selectedRoute = new WritableValue(null, Route.class);
		
		
		public Route getSelectedRoute() {
			return (Route)selectedRoute.getValue();
		}
		...
		private void bindRidgets(IRidgetContainer container) {
			...
			final ITextRidget textId = container.getRidget(ITextRidget.class,
					RIDGET_ID_COLLECTION_CENTRE_ID);
			...
			final ITextRidget description = container.getRidget(
					ITextRidget.class, RIDGET_ID_DESCRIPTION);
			description.bindToModel(workingCopy, "description");
			description.updateFromModel();
			description.setMandatory(true);

			final ITextRidget phone = container.getRidget(ITextRidget.class,
					RIDGET_ID_PHONE);
			phone.bindToModel(workingCopy, "phone");
			phone.updateFromModel();

			final IDateTextRidget dateOpened = container.getRidget(
					IDateTextRidget.class, RIDGET_ID_DATEOPENED);
			dateOpened.setFormat(DATE_FORMATE);
			dateOpened.bindToModel(workingCopy, "dateOpened");
			dateOpened.updateFromModel();

			final IMultipleChoiceRidget functions = container.getRidget(
					IMultipleChoiceRidget.class, RIDGET_ID_FUNCTIONS);
			final IObservableList optionValues = new WritableList(
					Arrays.asList(DairyFunction.values()), DairyFunction.class);
			final IObservableList selectionValues = new WritableList(
					workingCopy.getFunctions(), DairyFunction.class);
			functions.bindToModel(optionValues, selectionValues);
			functions.updateFromModel();

			routeCombo = container.getRidget(IComboRidget.class,
					RIDGET_ID_ROUTE);
			bindRouteCombo();
		...
		private void configureValidators(IRidgetContainer container) {
			...
			if (textName.getValidationRules().size() <= 0) {
				textName.addValidationRule(nameValidator,
		...
		private void configureActionListeners(IRidgetContainer container) {
			...
			final IActionRidget configureRouteAction = container.getRidget(
					IActionRidget.class, RIDGET_ID_CONFIGURE_ROUTE_ACTION);
		...
		public boolean isChanged(Object source, Object target) {
			if (source != null && target != null) {
				...
				if (EMFUtil.compare(src, dst)
						...
						&& EMFUtil.compare(src.getLocation()
								.getPostalLocation(), dst.getLocation()
								.getPostalLocation())
						&& EMFUtil.compare(src.getLocation()
								.getDescriptiveLocation(), dst.getLocation()
								.getDescriptiveLocation())
						&& EMFUtil.compare(src.getLocation().getMapLocation(),
								dst.getLocation().getMapLocation())) {
					return false;
				}
			}
		...
		public DairyLocation createWorkingCopy() {
			...
			final PostalLocation postalLocation = ModelFactory.eINSTANCE
					.createPostalLocation();

			final DescriptiveLocation descriptiveLocation = ModelFactory.eINSTANCE
					.createDescriptiveLocation();

			final MapLocation mapLocation = ModelFactory.eINSTANCE
					.createMapLocation();
		...
		public Object copyBean(Object source, Object target) {
			...
			to.getLocation()
					.getPostalLocation()
					.setAddress(
							from.getLocation().getPostalLocation().getAddress());
			to.getLocation()
					.getPostalLocation()
					.setEstate(
							from.getLocation().getPostalLocation().getEstate());
			to.getLocation()
					.getPostalLocation()
					.setVillage(
							from.getLocation().getPostalLocation().getVillage());
			to.getLocation()
					.getPostalLocation()
					.setPostalCode(
							from.getLocation().getPostalLocation()
									.getPostalCode());
			to.getLocation()
					.getPostalLocation()
					.setProvince(
			...
			to.getLocation()
					.getPostalLocation()
					.setSection(
							from.getLocation().getPostalLocation().getSection());
			to.getLocation()
					.getPostalLocation()
					.setDivision(
							from.getLocation().getPostalLocation()
			...
			to.getLocation()
					.getMapLocation()
					.setLatitude(
							from.getLocation().getMapLocation().getLatitude());
			to.getLocation()
					.getMapLocation()
					.setLongitude(
		...
		private void configureAddressTab(IRidgetContainer container,
				DairyLocation dairyLocation) {
			textAddress = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_ADDRESS);
			textAddress.setMandatory(true);
			textAddress.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "address");
			textAddress.updateFromModel();

			final ITextRidget section = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_SECTION);
			section.bindToModel(
					dairyLocation.getLocation().getPostalLocation(), "section");
			section.updateFromModel();

			final ITextRidget town = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_TOWN);
			town.bindToModel(dairyLocation.getLocation().getPostalLocation(),
					"village");
			town.updateFromModel();

			final ITextRidget estate = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_ESTATE);
			estate.bindToModel(dairyLocation.getLocation().getPostalLocation(),
					"estate");
			estate.updateFromModel();

			final ITextRidget location = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_LOCATION);
			location.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "location");
			location.updateFromModel();

			final ITextRidget sub = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_SUB);
			sub.bindToModel(dairyLocation.getLocation().getPostalLocation(),
					"subLocation");
			sub.updateFromModel();

			final ITextRidget district = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_DISTRICT);
			district.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "district");
			district.updateFromModel();

			final ITextRidget division = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_DIVISION);
			division.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "division");
			division.updateFromModel();

			final ITextRidget postalCode = container.getRidget(
					ITextRidget.class, RIDGET_ID_PL_POSTALCODE);
			postalCode.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "postalCode");
			postalCode.updateFromModel();

			final ITextRidget province = container.getRidget(ITextRidget.class,
					RIDGET_ID_PL_PROVINCE);
			province.bindToModel(dairyLocation.getLocation()
					.getPostalLocation(), "province");
			province.updateFromModel();

		}

		private void configureDirectionsTab(IRidgetContainer container,
				DairyLocation dairyLocation) {
			final ITextRidget landmark = container.getRidget(ITextRidget.class,
					RIDGET_ID_DL_LANDMARK);
			landmark.bindToModel(dairyLocation.getLocation()
					.getDescriptiveLocation(), "landmarks");
			landmark.updateFromModel();

			final ITextRidget directions = container.getRidget(
					ITextRidget.class, RIDGET_ID_DL_DIRECTIONS);
			directions.bindToModel(dairyLocation.getLocation()
					.getDescriptiveLocation(), "directions");
			directions.updateFromModel();
		}

		private void configureMapTab(IRidgetContainer container,
				DairyLocation dairyLocation) {
			final ITextRidget latitude = container.getRidget(ITextRidget.class,
					RIDGET_ID_ML_LATITUDE);
			latitude.bindToModel(dairyLocation.getLocation().getMapLocation(),
					"latitude");
			latitude.updateFromModel();

			final ITextRidget longitude = container.getRidget(
					ITextRidget.class, RIDGET_ID_ML_LONGITUDE);
			longitude.bindToModel(dairyLocation.getLocation().getMapLocation(),
					"longitude");
		...
		private class RouteSelectCallback implements ISelectionListener {
			...
			public void ridgetSelected(SelectionEvent event) {
				if (event.getNewSelection().size() > 0) {
					final String selectedName = (String) event
							.getNewSelection().get(0);
					Route selectedRoute = null;
					for (Route test : locationRepository.getRoutes()) {
					...
					final IComboRidget select = (IComboRidget) event
							.getSource();
					if (selectedRoute == null) { // selection is invalid since
													// the route
			...
			public IStatus validate(Object value) {
				...
				for (int i = 0; i < locations.size(); i++) {
					if (textName.getText().equals(
							((DairyLocation) locations.get(i)).getName())

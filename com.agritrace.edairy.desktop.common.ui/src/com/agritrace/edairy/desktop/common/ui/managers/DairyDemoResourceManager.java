...
public class DairyDemoResourceManager implements IDairyResourceManager {
	...
	public void createFarmResource() {
		...
		// "/farmDB.farm");
		// farmResource =
		// ResourceManager.INSTANCE.createResource(farmResourceURI);
		// }
	}


	private Farm createFarm(long id, String name, Location farmLocation) {
	...
	private List<Farm> createFarms(int farmId) throws ParseException {
		...
		final Farm farm = createFarm(
				farmId,
				"Green Farm_",
				createLocation(
						createPostalLocation("2 - Ngeche", "Section A", "Building B", "West Windsor",
								"Princeton Junction", "Princeton", "Mercer", "Central", "Western", "08550"),
						createMapLocation(23.5d, 16.89d),
						createDescriptiveLocation("last house on the old lane", "Wendy's")));

		createFarmProperties(farm, 20, 20, 8000, 9000);
		farms.add(farm);

		final Farm farm1 = createFarm(
				farmId + 1,
				"Red Farm_",
				createLocation(
						createPostalLocation("3 - Bindaka", "Section C", "Building X", "East Windsor",
								"Forman Junction", "Gulticor", "Mercer", "Central", "Western", "08550"),
						createMapLocation(21.5d, 116.89d),
						createDescriptiveLocation("the last house on the old lane", "Wendy's")));
	...
	public void createDairyResource(String baseDir) throws ParseException {
		...
		Dairy theDairy = createTestDairy();
		dairyResource.getContents().add(theDairy);
	}
	
	public Dairy createTestDairy() throws ParseException {
		// location
		
		final Dairy testDairy = DairyFactory.eINSTANCE.createDairy();
		
		testDairy.setDairyId(1l);
		testDairy.setCompanyName("Test Dairy #1");
		testDairy.setLegalName("Local Dairy Test Co. Inc.");
		testDairy.setFederalPin("FP-123456-7890");
		testDairy.setLicenseEffectiveDate(dateFormatter.parse("1/1/2000"));
		testDairy.setLicenseExpirationDate(dateFormatter.parse("12/31/2020"));
		testDairy.setNhifNumber("NHIF-123-2345-123");
		testDairy.setNssfNumber("NSSF-123-12345-123");
		testDairy.setPhoneNumber("321-123-12345");
		testDairy.setRegistrationNumber("R-987-9874-123454");
		
		addTestLocations(testDairy);
		addEmployees(testDairy);
		
		return testDairy;
	}
	
	private void addEmployees(Dairy testDairy) {
		// TODO Auto-generated method stub
		
	}

	private void addTestLocations(Dairy toTest) throws ParseException {
		...
		final Date startDate = dateFormatter.parse("04/18/1998");
		...
		final Date startDate2 = dateFormatter.parse("02/03/1994");
		...
		final Date startDate3 = dateFormatter.parse("07/14/1994");
		...
		final Date acquiredDate = dateFormatter.parse("02/18/2008");
		...
		final Date acquiredDate2 = dateFormatter.parse("07/21/2007");
		...
		final Date acquiredDate3 = dateFormatter.parse("07/27/2007");
		...
		final Date openDate = dateFormatter.parse("04/27/2005");
		...
		final Date openDate2 = dateFormatter.parse("04/27/2005");
		...
		final Date openDate3 = dateFormatter.parse("04/27/2005");
		...
		final Date openDate2_1 = dateFormatter.parse("03/26/2008");
		...
		final Date openDate2_2 = dateFormatter.parse("03/26/2008");
		...
		final Date openDate2_3 = dateFormatter.parse("04/27/2008");
		...
		toTest.getEmployees().add(driver1);
		toTest.getEmployees().add(driver2);
		toTest.getEmployees().add(driver3);
		toTest.getVehicles().add(v1);
		toTest.getVehicles().add(v2);
		toTest.getVehicles().add(v3);
		toTest.getBranchLocations().add(dLocation);
		toTest.getBranchLocations().add(dLocation2);
		toTest.getBranchLocations().add(dLocation3);
		toTest.getBranchLocations().add(dLocation2_1);
		toTest.getBranchLocations().add(dLocation2_2);
		toTest.getBranchLocations().add(dLocation2_3);

		toTest.getRoutes().add(route1);
		toTest.getRoutes().add(route2);
		toTest.getDairyBins().addAll(bins);
		...
		for (int i = 0; i < 20; i++) {
			...
			final Date applicationDate = dateFormatter.parse(dateString);
			member1.setApplicationDate(applicationDate);

			final Date effectiveDate = dateFormatter.parse("03/05/2007");
			...
			toTest.getMemberships().add(member1);
		}
...
//		return toTest;
	...
	private void addSuppliers(Dairy dairy) throws ParseException {
		Person sharedContact = createPerson("Ronald", "G", "Lipshutz", "343432322", null, null);
		dairy.getSuppliers().add(
				createSupplier(10002l, "Sandy Carpet Ltd ", "SCL", VendorStatus.VALID,
						dateFormatter.parse("02/01/2011"), 
						Arrays.asList("Category 1", "Category 2"),
						Arrays.asList(sharedContact)));

		dairy.getSuppliers().add(
				createSupplier(10002l, "Tom  Amusement Ltd ", "TAL", VendorStatus.VALID,
						dateFormatter.parse("02/01/2011"), 
						Arrays.asList("Category 1", "Category 2"),
						Arrays.asList(sharedContact, createPerson("Sam", "", "MacClane", "845-667-77889"))));
		...
		dairy.getSuppliers().add(
				createSupplier(
						...
						dateFormatter.parse("02/01/2011"),
						Arrays.asList("Category 1", "Category 2"),
						Arrays.asList(createPerson("Andy", "G", "Cliboff", "342432322", null, null),
								createPerson("Simeon", "G", "Pinder", "1122334455", null, null))));
	...
	public void loadFarmResources() {
		...
		// "/farmDB.farm");
		// farmResource =
		// ResourceManager.INSTANCE.loadResource(farmResourceURI);
	}
	...
	private void createFarmProperties(Farm farm, int containerNumber, int animalNumber, int containerId, int animalId)
		...
		for (int i = 0; i < animalNumber; i++) {
			...
			final Date effectedDate = dateFormatter.parse(date);

...
public abstract class CommonLookupDialog extends AbstractDialogView {
	...
	private void createButtons(Composite composite) {
		final Button selectBtn = UIControlsFactory.createButton(composite, "&Select", SELECT_BUTTON);
		GridDataFactory.swtDefaults().span(4, 1).align(SWT.CENTER, SWT.BEGINNING).applyTo(selectBtn);

	}

	@Override
	protected Control buildView(Composite parent) {
		final Composite composite = UIControlsFactory.createComposite(parent, SWT.NULL);
		...
		// Text value
		final Text lookupValue = UIControlsFactory.createText(composite, SWT.NULL | SWT.BORDER | SWT.SINGLE,
				SEARCH_COLUMN_TEXT);
		...
		final Table table = UIControlsFactory.createTable(panel, SWT.FULL_SELECTION | SWT.BORDER | SWT.MULTI,
				RESULT_LIST);
